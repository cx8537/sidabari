...
import { COLLECT_COMMAND, SERVICE_NAME } from "@/lib/diagnostic";
...
export function MainClaudePanel() {
  ...
  async function handleSystemCollect() {
    ...
    // 그래도 우리 의도를 명시적으로 전달하기 위해 COLLECT_COMMAND를 보냄 (서버는 무시하고 자기 스크립트 실행).
    const sshCmd = `ssh -i "${diagKey}" -p ${ec2.port} -o StrictHostKeyChecking=accept-new -o BatchMode=yes ${ec2.user}@${ec2.host} '${COLLECT_COMMAND}'`;

    const prompt = [
      `${SERVICE_NAME} 시스템 진단 데이터를 수집해 분석해 주세요. 본 작업은 read-only 진단입니다.`,
      "",
      "[안전 전제 — 반드시 숙지]",
      ...
      >
        ...
        <div className="flex items-center gap-1.5">
          <Button
            ...
            disabled={!mainClaudeSessionId || !diagKeyConfigured}
            className="[&_svg]:text-action-green"
            title={
              !mainClaudeSessionId
                ? "메인 Claude 세션이 비활성"
                : !diagKeyConfigured
                  ? "진단 전용 키 미설정 — 설정 모달의 [Claude 시스템 진단 안전장치] 섹션 참조 (docs/ec2-diag-setup/README.md)"
                  : `Claude에게 ${SERVICE_NAME} 시스템 진단을 진단 전용 키로 수행 요청 (read-only · 입력창에 주입 후 검토)`
            }
          >
            <Activity /> 시스템 데이터 수집
          </Button>
